      ...
      {blocks.map((block, i) => {
        ...
        switch (block.type) {
          case "heading": {
            ...
            return <Tag key={i} className={`${sizes[block.level] || sizes[3]} font-medium leading-[1.3]`} style={{ fontFamily: t.serif, color: t.ink(0.85) }}>{block.text}</Tag>;
          }
          case "paragraph":
            return <p key={i} className="text-[15px] md:text-[16px] leading-[1.9]" style={{ fontFamily: t.sans, color: t.ink(0.55) }}>{block.text}</p>;
          case "image":
            return (
