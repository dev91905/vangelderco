    ...
    <AtmosphericLayout>
      ...
      <div className="relative z-20 h-dvh overflow-y-auto" style={{ animation: "fade-up 0.5s ease-out both" }}>
        <div className="w-full flex flex-col items-center justify-end px-6 pb-8" style={{ minHeight: "40vh" }}>
          ...
          <h1 className="text-[28px] md:text-[40px] font-medium leading-[1.15] text-center max-w-3xl mb-4" style={{ fontFamily: t.serif, color: t.ink(0.9), animation: "fade-up 0.5s ease-out 0.3s both" }}>
            {post.title}
          </h1>

          {post.excerpt && (
            <p className="text-[15px] leading-[1.7] text-center max-w-2xl mb-4" style={{ fontFamily: t.sans, color: t.ink(0.5), animation: "fade-up 0.5s ease-out 0.35s both" }}>
              {post.excerpt}
            </p>
          )}
        ...
        <main className="flex flex-col items-center px-6 pb-20 max-w-[720px] mx-auto gap-8">
          ...
          <article className="w-full" style={{ animation: "fade-up 0.6s ease-out 0.7s both" }}>
            {post.content_blocks ? <ContentBlockRenderer blocks={post.content_blocks} renderExtended={renderExtended} /> : (
              <p className="text-[15px] leading-[1.9]" style={{ fontFamily: t.sans, color: t.ink(0.4) }}>No content available.</p>
            )}
          </article>
        </main>
      </div>
