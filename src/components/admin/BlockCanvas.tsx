    ...
    <div className="space-y-0 relative">
      ...
      {safeBlocks.length === 0 && (
        <div className="flex flex-col items-center justify-center py-24 gap-4">
          <p className="text-sm" style={{ fontFamily: t.sans, color: t.ink(0.25) }}>Start writing</p>
          <button
            onClick={() => setPickerIndex(0)}
            className="p-3 rounded-full transition-all hover:scale-105"
            style={{ background: t.ink(0.04), border: t.border(0.1) }}
          >
            <Plus className="w-5 h-5" style={{ color: t.ink(0.4) }} />
          </button>
        </div>
      )}
      ...
      {isMobile && safeBlocks.length > 0 && (
        <button
          ...
          style={{ background: t.ink(0.03), border: t.border(0.06) }}
        >
          <Plus className="w-4 h-4" style={{ color: t.ink(0.3) }} />
          <span className="text-[11px]" style={{ fontFamily: t.sans, color: t.ink(0.3) }}>Add block</span>
        </button>
      )}
    </div>
  );
...
function InsertPoint({ index, pickerIndex, setPickerIndex, onInsert, isCaseStudy }: {
  ...
  return (
    <div ref={ref} className="relative group flex items-center justify-center" style={{ height: "12px" }}>
      <div className="absolute inset-x-6 top-1/2 h-px opacity-0 group-hover:opacity-100 transition-opacity duration-300" style={{ background: t.ink(0.06) }} />
      <button
        ...
        style={{ background: t.ink(0.05), border: t.border(0.1) }}
      >
        <Plus className="w-3 h-3" style={{ color: t.ink(0.5) }} />
      </button>
      {isOpen && (
